# ROS2 Gazebo Robot

A differential drive robot built from scratch using ROS2 Humble and Gazebo, running on Ubuntu 22.04 via WSL2.

## Project Phases
- [x] Phase 0 — WSL2 + Ubuntu 22.04 + ROS2 Humble setup
- [x] Phase 1 — ROS2 workspace and core concepts
- [x] Phase 2 — URDF robot description visualized in RViz
- [ ] Phase 3 — Launching in Gazebo simulation
- [ ] Phase 4 — ROS2 nodes for robot control
- [ ] Phase 5 — Lidar sensor and obstacle avoidance
- [ ] Phase 6 — Custom Gazebo world

## Stack
- ROS2 Humble
- Gazebo
- Python
- URDF/SDF
